var Follow = function () {
	...
	init.prototype = {
		...
		start : function () {
			...
			for(var i=0;i<660;i+=20){
				var O = OO;
				for(var j=10; j<16; j+=1){
					var x = fn(i, j).x,
					y = fn(i, j).y;
			...
			setInterval(function() {for (var i = 0; i < N; i++) OBJ[i].run();}, 16);
		}
	}
	...
	var CObj = function (p, cx, cy) {
		...
		this.css = obj.style;
		this.css.background = '#'+parseInt(Math.random()*9)+parseInt(Math.random()*9)+parseInt(Math.random()*9);
		this.css.fontSize = 0;
...
onload = function(){
	...
	new Follow('screen', {
		...
		animR: 1,
		fn : function (i, j){
			return {x : j/4*Math.cos(i),y : j/4*Math.sin(i)}
		}
	});
}